...
// Default for @list slot - shown when navigating to /recipes/[id]
// Now also respects search params to maintain filtered results
export default async function ListDefault({ searchParams }: ListDefaultProps) {
  const { q: query } = await searchParams;
  const initialData = await getCachedSearchRecipes({ query, limit: 9 });
